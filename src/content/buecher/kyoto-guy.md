---
# Angelegt aus den beiden Buchsatz-Dateien: „Opium" (2.062 Woerter) und
# „The Kyoto Guy" (3.865 Woerter). Klappentexte, Zitate und Leseproben
# stehen im Wortlaut der Manuskripte.
#
# Seite A und B wurden getauscht: „Opium" liegt jetzt vorn.
titel: Opium
kurztitel: Opium
autor: C. T. Selters
# Der Klappentext hört auf, bevor er verrät, was das „letzte Werk" ist.
# Hier stand einmal der Satz „Es ist kein Buch, sondern ein Parfum — und
# eine Note verrät der Alte nicht." Das ist die Wendung des Textes; wer
# sie im Klappentext liest, liest sie nicht mehr im Buch.
klappentext: >-
  Ein Wiener Sammler hört von einem verschollenen Autor kolonialer
  Reiseberichte, bucht noch am selben Nachmittag den Flug nach Kunming und
  kauft ihm sein letztes Werk ab.
klammer: >-
  Ein Sammler fliegt nach Kunming und kauft einem Alten sein letztes Werk
  ab; ein Gast in Gion wird über acht Wochen zum Fall. Zwei Männer, die
  etwas suchen und etwas anderes finden.
zitat: >-
  Sie lieben das Abenteuer, deswegen werde ich Ihnen die letzte Note nicht
  verraten.
zitat_von: Christian Sava
# Vorlaeufig: das Braun der Pfeifen, das Gold des Flakons, Elfenbein.
cover_bild: /buecher/kyoto-guy/opium-2.webp
cover_farbe: '#241a12'
akzent_farbe: '#c9a24a'
schrift_farbe: '#e8dcc4'
motiv: continuum
leseprobe:
  # GESCHAETZT: gegen Ende der zweiten Erzaehlung.
  seite: 6
  text: >-
    Nach drei seltsamen gemeinsamen Tagen, die hauptsächlich aus Tee,
    Opium, Tee, Opiumtee und Opium bestanden, nur unterbrochen durch
    Episoden unbehaglicher intimer Gespräche und meinen kläglichen
    Versuchen, die lokalen Hmong-Arbeiter mit einer Reihe antiquierter
    vietnamesischer Fachwörter für Opium-Paraphernalien auf meine Seite zu
    ziehen, hatten wir uns auf den Preis von umgerechnet 1200 Dollar für
    sein letztes Werk geeinigt und uns die Hand darauf gegeben, als Sava
    wie nebenbei einstreute, dass es sich bei Oh Unknown tatsächlich um
    keinen Roman, sondern um [[eine „transzendente Parfumkreation"]]
    handle, die die Essenz aller seiner „profunden Teeerfahrungen der
    letzten Jahrzehnte" beinhalten sollte; ich war perplex angesichts des
    ungeheuerlichen Ausmaßes der Nutzlosigkeit meiner Reise und der
    grausamen 72 Stunden von Zhongdian; doch nach all den Strapazen war
    ich in keiner Stimmung mehr zu verhandeln, also nahm ich das Parfum
    dankend in Empfang und sprühte mir damit in einer Kreuzbewegung unter
    die Achseln und über die verschwitzte Knopfleiste meines Hemdes; ein
    nicht unangenehmer Geruch breitete sich aus, der mich für einige
    Augenblicke lang besänftigen konnte.


    Die Stimmung kippte schließlich, als Sava mein Geld in Empfang
    genommen hatte und mir mit dem stolzen Trotz eines alten Mannes
    mitteilte, dass das Parfum neben den von ihm aufgezählten Duftstoffen
    noch eine weitere Geheimzutat enthalten würde, die er mir jedoch um
    nichts in der Welt verraten würde; „Sie, Herr Selters", sagte mir
    dieser unverschämte Mensch, „lieben das Abenteuer, deswegen werde ich
    Ihnen [[|17]]
format: Broschur
verfuegbarkeit: In Vorbereitung
hoehe: 2.0
dicke: 0.078
reihenfolge: 4
# Wendeband: auf der Rückseite die zweite Geschichte, kopfüber gedruckt.
rueckseite:
  # Angelegt aus den beiden Buchsatz-Dateien: „The Kyoto Guy" (3.865 Woerter)
  # und „Opium" (2.062 Woerter). Klappentexte, Zitate und Leseproben stehen
  # im Wortlaut der Manuskripte.
  #
  titel: The Kyoto Guy
  kurztitel: The Kyoto Guy
  autor: Christoph Laible
  klappentext: >-
    Datierte Einträge aus Kyoto: eine Wohnung in Gion, ein Club unter einem
    Neonschmetterling, ein zwei Jahre altes Wiesn-Foto mit einer goldenen
    Spielzeugpistole. Zwischen dem achten September und dem fünfzehnten
    Oktober wird aus einem Gast ein Fall.
  zitat: Das war eine Spielzeugpistole! Ein Dekoobjekt.
  zitat_von: Eintrag 15.09.2025
  # Gesetzt in Times New Roman — wie die uebrigen Baende der Reihe.
  leseprobe_schrift: "'Times New Roman', Times, serif"
  # GESCHAETZT: beide Erzaehlungen zusammen rund 38.000 Zeichen, also etwa
  # zwanzig Seiten Text. Bitte aus dem Umbruch nachtragen.
  seiten_zahl: 32
  # Vorlaeufig, bis der Umschlag da ist: das Nachtgruen des Bambus, das
  # Zinnober der Tore, Knochenweiss fuer die Schrift.
  cover_bild: /buecher/kyoto-guy/cover.webp
  cover_farbe: '#1b211c'
  akzent_farbe: '#c4442e'
  schrift_farbe: '#ded6c4'
  motiv: steps
  leseprobe:
    # GESCHAETZT wie oben: die Stelle liegt bei rund der Haelfte.
    seite: 9
    text: >-
      Ich kaue auf den Fingernägeln, schaue aus dem Fenster meiner Wohnung und
      warte darauf, dass Eddy endlich abnimmt. Oder wenigstens Marie kommt.
      Mein Instagram-Postfach explodiert. Und dann gibt es diese
      Wordpress-Seite ohne Impressum. Mit meiner Adresse. Mit der Adresse
      meiner Eltern. Ein Artikel ist mit [[Völkischer Waffennarr]] betitelt.
      Dazu ein Foto, das der Betreiber von meiner Instagram-Seite gerippt hat:
      ich im Trachtenjanker, [[eine goldene Pistole in der Hand]]. Das war eine
      Spielzeugpistole! Ein Dekoobjekt. Aufgenommen in der Wohnung von Jürgen
      und Michael, zwei Jahre her, nach der Wiesn. Ich laufe im Wohnzimmer
      auf und ab, setze mich, stehe wieder auf, setze mich. Ich könnte mir
      die Haare abrasieren und mir einen Schnauzer wachsen lassen, denke ich.
      Ein Glatzentattoo stechen. Eine Japanerin heiraten, die sich nicht an
      dem Video stört. [[Die Japaner nehmen einem die Vergangenheit nicht]].


      15.09.2025


      Ich höre das Schloss und Marie tritt in meine Wohnung. „Marie, ich kann
      Gegenwind aushalten. Echt. Wenn ich hinter was stehe, dann halte ich das
      aus." Meine Stimme ist heiser. „Aber das war eine Suffaktion. Dass mein
      Leben versaut ist wegen was, hinter dem ich nicht stehe." Marie stellt
      ihre Tasche ab, zieht die Schuhe aus. „Du kennst mich", sage ich. „Wenn
      mich jetzt alle hassen, weil ich [[|17]]
---

Ein Band, zwei Vorderseiten. Vorn „Opium" von C. T. Selters —
Gion, ein Club, ein altes Foto und die Wochen danach, in datierten
Einträgen. Gewendet: „The Kyoto Guy" von Christoph Laible — Wien, Kunming, Zhongdian,
ein Sammler und ein alter Mann, der eine Note für sich behält.
